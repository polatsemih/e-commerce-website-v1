                                        ...
                                        if ($authenticated_user_from_database['data']['is_user_blocked'] == 0) {
                                            ...
                                            if ($conversation_id['result'] && $last_conversation_id['result']) {
                                                if (!empty($_POST['mdStatus']) && !empty($_POST['status']) && !empty($_POST['conversationId'])) {
                                                    if ($_POST['mdStatus'] == 1 && $_POST['status'] == 'success') {
                                                        if ($last_conversation_id['data']['conversation_id'] == $_POST['conversationId']) {
                                                            if (!empty($_POST['paymentId'])) {
                                                                    ...
                                                                    if ($threedsPayment->getStatus() == 'success') {
                                                                        if ($threedsPayment->getConversationId() == $conversation_id['data']) {
                                                                            ...
                                                                            if (!$result_create_order_payment['result']) {
                                                                                ...
                                                                                $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'OrderPaymentDBError'));
                                                                            }
                                                                            ...
                                                                            foreach ($threedsPayment->getPaymentItems() as $key => $payment_item) {
                                                                                ...
                                                                                if (!$result_create_item_transaction['result']) {
                                                                                    ...
                                                                                    $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'OrderPaymentTransactionDBError'));
                                                                                }
                                                                                ...
                                                                                if ($ordered_item['result'] && $ordered_item_price['result']) {
                                                                                    ...
                                                                                    if ($item_sold['result']) {
                                                                                        ...
                                                                                        if (!$result_update_item['result']) {
                                                                                            ...
                                                                                            $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'OrderPaymentUpdateDBError'));
                                                                                        }
                                                                                    }
                                                                                }
                                                                            }
                                                                            ...
                                                                            if (!empty($order_mail_items)) {
                                                                                ...
                                                                                $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'UserNewOrder'));
                                                                            }
                                                                            $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'AdminNewOrder'));
                                                                ...
                                                                $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'OrderVerifyMissingPaymentId'));
                                                                $this->notification_control->SetNotification('DANGER', TR_NOTIFICATION_ERROR_DATABASE);
                                                                $this->input_control->Redirect(URL_CART);
                                                            }
                                                        } else {
                                                    ...
                                                    $this->ActionModel->CreateLogEmailSent(array('user_id' => $authenticated_user_from_database['data']['id'], 'user_ip' => $_SERVER['REMOTE_ADDR'], 'email_type' => 'OrderVerifyMissingPostDatas'));
                                                    $this->notification_control->SetNotification('DANGER', TR_NOTIFICATION_ERROR_DATABASE);
                                                    $this->input_control->Redirect(URL_CART);
                                                }
                                            } else {
