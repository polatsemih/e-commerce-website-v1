...
class AccountController extends Controller
{
    function __construct()
    {
        ...
        if (empty($this->web_data['authenticated_user'])) {
            ...
            $is_user_blocked = $this->UserModel->GetUserByUserId('is_user_blocked', $this->web_data['authenticated_user']);
            if ($is_user_blocked['result']) {
                if ($is_user_blocked['data']['is_user_blocked'] == 1) {
                    echo '{"reset":true}';
                    exit(0);
                }
            } else {
