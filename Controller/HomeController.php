...
class HomeController extends Controller
{
        ...
        try {
            if ($_SERVER['REQUEST_METHOD'] === 'POST' || $_SERVER['REQUEST_METHOD'] === 'GET') {
                ...
                if (!empty($selected_gender)) {
                    parent::LogView('Home-Items_' . $selected_gender['gender_url']);
        ...
        try {
            if ($_SERVER['REQUEST_METHOD'] === 'POST' || $_SERVER['REQUEST_METHOD'] === 'GET') {
                ...
                if ($item_details_from_database['result'] && $sizes_from_database['result']) {
                    ...
                    if (!empty($selected_gender)) {
                        parent::LogView('Home-ItemDetails_' . $item_details_from_database['data']['item_url']);
                        $this->web_data['genders'] = $genders_from_database;
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                ...
                switch ($agreement_url) {
                    ...
                    case URL_AGREEMENT_RETURN_POLICY:
                        $case_matched = true;
                        parent::LogView('Home-Agreements_' . URL_AGREEMENT_RETURN_POLICY);
    ...
    function Profile(string $profile_url)
    {
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $this->input_control->CheckUrl();
                parent::LogView('Home-Profile_' . $profile_url);
                $case_matched = false;
                switch ($profile_url) {
